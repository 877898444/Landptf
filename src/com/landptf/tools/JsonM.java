...
public class JsonM {
     ...
     * @return json类型字符串 
     */  
    public static String toJSON(Object obj, boolean classParentFlag) {  
        JSONStringer js = new JSONStringer();  
        serialize(js, obj, classParentFlag);  
        return js.toString();  
    }
     ...
     * @param o 待需序列化的对象 
     */  
    private static void serialize(JSONStringer js, Object o, boolean classParentFlag) {
        if (isNull(o)) {  
            try {
                js.value(null);  
        ...
        if (isObject(clazz)) { // 对象  
            serializeObject(js, o, classParentFlag);  
        } else if (isArray(clazz)) { // 数组  
            serializeArray(js, o, classParentFlag);
        } else if (isCollection(clazz)) { // 集合  
            Collection<?> collection = (Collection<?>) o;  
            serializeCollect(js, collection, classParentFlag);  
        }else if (isMap(clazz)) { // 集合  
            HashMap<?,?> collection = (HashMap<?,?>) o;  
            serializeMap(js, collection, classParentFlag);  
        } else { // 单个值  
            try {
                js.value(o);  
            } catch (JSONException e) {
     ...
     * @param array 数组 
     */  
    private static void serializeArray(JSONStringer js, Object array, boolean classParentFlag) {  
        try {  
            js.array();  
            for (int i = 0; i < Array.getLength(array); ++i) {  
                Object o = Array.get(array, i);  
                serialize(js, o, classParentFlag);  
            }  
            js.endArray();  
        } catch (Exception e) {  
            e.printStackTrace();  
     ...
     * @param collection    集合 
     */  
    private static void serializeCollect(JSONStringer js, Collection<?> collection, boolean classParentFlag) {  
        try {  
            js.array();  
     ...
     * @param map   map对象 
     */  
    private static void serializeMap(JSONStringer js, Map<?,?> map, boolean classParentFlag) {  
        try {  
            js.object();  
            ...
            while (it.hasNext()) {  
                ...
                js.key(entry.getKey());  
                serialize(js,entry.getValue(),classParentFlag);  
            }  
            js.endObject();  
        } catch (Exception e) {  
            e.printStackTrace();  
     ...
     * @param obj   待序列化对象 
     */  
    private static void serializeObject(JSONStringer js, Object obj, boolean classParentFlag) {  
        try {
        	js.object();
        	...
        	Method[] methods;
        	Field[] fields;
        	Method[] methodsParent = new Method[0];
        	Field[] fieldsParent = new Field[0];
        	Method[] methodsChild = objClazz.getDeclaredMethods();     
        	Field[] fieldsChild = objClazz.getDeclaredFields();
        	if (classParentFlag) {
        		methodsParent = objClazz.getSuperclass().getDeclaredMethods();
        		fieldsParent = objClazz.getSuperclass().getDeclaredFields();
			}
        	methods = new Method[methodsChild.length + methodsParent.length];
        	fields = new Field[fieldsChild.length + fieldsParent.length];
        	System.arraycopy(methodsChild, 0, methods, 0, methodsChild.length);
        	System.arraycopy(methodsParent, 0, methods, methodsChild.length, methodsParent.length);
        	System.arraycopy(fieldsChild, 0, fields, 0, fieldsChild.length);
        	System.arraycopy(fieldsParent, 0, fields, fieldsChild.length, fieldsParent.length);
            for (Field field : fields) {     
                try {     
                    ...
                    if (isObject(field.getType())) {
                    	serializeObject(js, fieldVal, classParentFlag);
                    	continue;
                    }
                    serialize(js, result, classParentFlag);
                } catch (Exception e) {     
                    continue;
                }     
            }
     ...
     * @return true或者false 
     */  
    private static boolean haveMethod(Method[] methods, String fieldMethod) {  
	...
	private static String parseMethodName(String fieldName,String methodType) {  
    ...
    public static <T> T parseObject(JSONObject jo, Class<T> clazz) throws JSONException {  
        return parseObject(jo, clazz,false);  
    } 
     ...
     * @throws JSONException  
     */  
    public static <T> T parseObject(JSONObject jo, Class<T> clazz, Boolean classParentFlag) throws JSONException {  
        ...
        if(isMap(clazz)){   
        	...
        	Method[] methodsParent = new Method[0];
        	Field[] fieldsParent = new Field[0];
        	Method[] methodsChild = clazz.getDeclaredMethods();     
        	Field[] fieldsChild = clazz.getDeclaredFields();
        	if (classParentFlag) {
        		methodsParent = clazz.getSuperclass().getDeclaredMethods();
        		fieldsParent = clazz.getSuperclass().getDeclaredFields();
			}
        	methods = new Method[methodsChild.length + methodsParent.length];
        	fields = new Field[fieldsChild.length + fieldsParent.length];
        	System.arraycopy(methodsChild, 0, methods, 0, methodsChild.length);
     ...
     * @throws JSONException  
     */  
    public static <T> T parseObject(String jsonStr, Class<T> clazz, Boolean classParentFlag) throws JSONException {  
        ...
        if (classParentFlag) {
        	return parseObject(jo, clazz,true);  
		}
        else {
    ...
    public static <T> T[] parseArray(JSONArray ja, Class<T> clazz) {  
        return parseArray(ja,clazz,false);  
    }  
     ...
     * @return  反序列化后的数组 
     */  
    public static <T> T[] parseArray(JSONArray ja, Class<T> clazz, Boolean classParentFlag) {  
        ...
        for (int i = 0; i < len; ++i) {  
            try {  
                ...
                T o = parseObject(jo, clazz,classParentFlag);  
                array[i] = o;  
            } catch (JSONException e) {  
                e.printStackTrace();  
            }  
    ...
    public static <T> T[] parseArray(String jsonStr, Class<T> clazz) {  
        return parseArray(jsonStr, clazz, false);  
    }  
     ...
     * @return  序列化后的数组 
     */  
    public static <T> T[] parseArray(String jsonStr, Class<T> clazz, Boolean classParentFlag) {  
        if (clazz == null || jsonStr == null || jsonStr.length() == 0) {  
            return null;  
        }  
        JSONArray jo = null;  
        ...
        return parseArray(jo, clazz,classParentFlag);  
    }  
